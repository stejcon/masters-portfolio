...
#figure(
  image("./images/onnx-mlir-architecture.png", width: 40%),
  caption: [
    The architecture of ONNX-MLIR, shown as Figure 2 in @onnxmlir
  ],
) <onnxmlirarchitecture>

The above methods all focused on optimising a model after it has been developed an trained, but other methods also exist. Neural architecture search is a method by which a neural network designs a new neural network for a given dataset. An example of this is provided by #cite(form: "normal", <NAS>), which utilises a recurrent neural network to provide designs for convolutional neural networks on particular datasets. This method tended to produce more accurate models, but these models were also much larger in terms of parameter count than manually-developed alternatives. Results from trying many handcrafted models as well as the generated models on the CIFAR-10 dataset are shown in @nasresults.
