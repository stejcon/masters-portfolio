...
A compile-time implementation for the automatic addition of exits was the first approach analysed. As discussed in Appendix A, ONNX-MLIR is a compiler for models which can allow them to run on any LLVM-supported architecture, which is preferable to a framework-specific approach. It exposes multiple passes over an ONNX model, but the most useful for editing the model itself is the first pass, `ONNXOpTransformPass`. This also for manipulation of the forward graph of the model for operations such as constant propagation and shape inference. These operations use rules written in LLVM's TableGen domain-specific language. To add the exit structures to the model, TableGen rules could be used to identify blocks within a CNN, as blocks generally contain a convolution, an averaging operation, and possibly another non-linear layer such as the rectified linear unit (ReLU).
...
To begin the implementation, it was chosen to use ResNet34 as the base model, trained on the CIFAR10 dataset. This combination has the benefit of both being relatively quick to train to the available hardware, taking approximately 1.5 hours to train the model for 20 epochs on a Nvidia 3070. The implementation of ResNet34 was taken from the TorchVision library included in PyTorch @TorchVision. The model was trained multiple time with varying hyperparameters to identify the hyperparameters which cause convergence the quickest, which are shown in @hyperparameters.
...
This structure was based on the pooling block as described in @earlyexitmasters and the normal exit of a ResNet model as described in @ResNet. The value of 0.5 for the entropy threshold was taken from @BranchyNet were it was used as a default threshold before more sophisticated approaches were presented. The return for the exit, as well as all exits for this implementation, also return an exit ID, also referred to as a return ID in the implementation, to indicate what exit was used. 0 is used to indicate the original backbone model, and any other positive integer indicates which exit was used, with a higher number indicating a deeper exit. This would not be strictly needed in a production-ready implementation. However, to compare how often each exit is used for the purposes of testing, it is important to have a return ID.
...
Once the exit was trained, the performance of the model dramatically fell, with both the final exit and early exit having an accuracy of approximately 10%. This occurred irrespective of the chosen hyperparameters. This singular issue stemmed from a multitude of issues with the above naive approach.

+ The layers for the early exit were identical to when they were initialised, indicating that the backwards propagation was not updating these layers.
...
To enable easier debugging, the Python code equivalent to the AST was saved to a file using the `ast.unparse()` function. This function takes an AST as input, and produces Python code which corresponds to the AST. This code is then written to a file to verify whether the entropy threshold was correctly set. `ast.unparse()` has some limitations, in particular it can fail if the AST being unparsed is too complex in terms of depth, and it also isn't guaranteed to perfectly invert `ast.parse()`. That is, `ast.unparse()` can produce different code to the code which originally produced the AST. Neither of these limitations come into play for this implementation however as exits are kept simple and there is no original code being parsed, exits are produced directly from a prewritten AST.
...
This class holds a reference to the model and all of the parameters used to create the model, including the actual model class, any arguments passed to the constructor and any extra needed parameters. Whenever code needs to make use of the model, it should be accessed through a `ReloadableModel` instance, as that guarantees that the model being used is the reloaded version. Model creation is changed from 
...
== Untrainable Layers
The weights for the exit layers still do not train correctly with this set up. This is due to a quirk of PyTorch's autograd engine which completes the backpropagation. Only layers included in a models `named_parameters` list are updated in the backpropagation. However no layers defined in the `forward` function, as was done with the linear layers as shown in @exitcode, are included in the `named_parameters` list. Only layers which have declared in the `__init__` function are updated. To solve this, the same methods used to update the `forward` function in `ExitTracker` are used to update the `__init__` function. `EarlyExit` was also modified to keep track of the layers which are needed for it's exit, and could modify the name of the layer from `self.exitX` to `self.exitY` to correspond to an updated ID. This would allow the freezing of the backbone model to continue to work while keeping the layers separated. This is not limited to single instructions and it is possible to have deeper exits with multiple layers for each exit as `EarlyExit` stores a list of layers which it needs.

When creating the `exitX` layers, the naive solution computed the dimensions needed manually. As every instruction may require slightly different treatment to calculate it's dimensions, this became complicated. To solve this, lazy layers were used instead. These are an experimental feature in PyTorch where the layers can calculate their own input dimensions. This requires a model to be run with a sample inference of the expected input dimensions for runtime samples, and the layers will then be initialised with the correct dimensions. This is also added to the reloading functionality of the `ReloadableModel`, and even if the model doesn't contain lazy layers, the model will still function. While still experimental, lazy layers makes the possibility of deep early exits easy to implement with simply a different list of AST nodes needing to be given to an `EarlyExit` instance.

== Setting Threshold Levels
There is no clear consensus on what method is best for choosing an entropy threshold. @BranchyNet simply uses 0.5 as the threshold for all branches in it's implementation, whereas @earlyexitmasters uses a trainable threshold. The method developed to set the threshold for exits in this implementation is based on one assumption: no exit should be expected to achieve a higher accuracy than that of the backbone model. After running the testing data through an exit, a set of graphs can be created, plotting accuracy against entropy. An example of an exits accuracy against entropy is shown in @accent.
...
This solution is significant as no literature has dealt with the issues involved in automatically adding exits to a model. Many of the challenges described above have not been discussed or documented, meaning the issues which appeared throughout the implementation of the above features were unexpected and were major blockers as they had been undiscovered. This solution lays the general groundwork for how exits should be automatically added, with most features being isolated and easy to change. To implement deeper exits, a different set of nodes can be sent to instances of `EarlyExit`. To provide searching for exits, all code can be encapsulated in `ExitTracker`. The approach described above, while implemented on PyTorch, should be relatively easy to reimplement on other frameworks.
...
= Source Code
The source code for this implementation can be found on GitHub#footnote(link("https://github.com/stejcon/masters-portfolio")). `EarlyExit`, `ExitTracker` and all nodes related to exit generation and addition can be found in `generation.py`. `ReloadableModel` and all functions used to train models can be found in `helpers.py`. `main.py` is where the actual training was done, but this was mainly used as a live script and frequently changed. It focused on constructing a `ReloadableModel` and then calling functions from `helpers.py` to train the stored model.
